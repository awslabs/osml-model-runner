...
class TestEnhancedRegionHandlerAsyncIntegration(unittest.TestCase):
    ...
    def test_setup_async_worker_pool_success(
        self, mock_pool_class, mock_metrics_class, mock_config_class, mock_detector_class
    ):
        ...
        mock_detector_class.assert_called_once_with(
            endpoint=self.region_request.model_name, assumed_credentials=None, async_config=mock_config
        )
        mock_metrics_class.assert_called_once()
        mock_pool_class.assert_called_once_with(
            async_detector=mock_detector, config=mock_config, metrics_tracker=mock_metrics
        )
    ...
    @patch("osml_extensions.handlers.enhanced_region_handler.AsyncMetricsTracker")
    @patch("osml_extensions.handlers.enhanced_region_handler.AsyncTileWorkerPool")
    def test_setup_async_worker_pool_with_credentials(
        self, mock_pool_class, mock_metrics_class, mock_config_class, mock_detector_class, mock_get_creds
    ):
        ...
        mock_detector_class.assert_called_once_with(
            endpoint=self.region_request.model_name, assumed_credentials=mock_credentials, async_config=mock_config
        )
    ...
    @patch("osml_extensions.handlers.enhanced_region_handler.AsyncEndpointConfig")
    @patch("osml_extensions.handlers.enhanced_region_handler.AsyncSMDetector")
    def test_setup_enhanced_tile_workers_async(
        self, mock_detector_class, mock_config_class, mock_metrics_class, mock_pool_class
    ):
    ...
    def test_process_tiles_with_async_pool(
        self, mock_create_tile, mock_tile_factory_class, mock_gdal_env_class, mock_tempfile, mock_queue_class
    ):
    ...
    def test_process_tiles_with_async_pool_no_tiles(self):
        ...
        total_count, failed_count = self.handler._process_tiles_with_async_pool(
            mock_async_pool, self.mock_tiling_strategy, mock_region_request_item, Mock(), None, None
        )
