    ...
    def test_find_features_success(self, mock_boto3):
        ...
        with patch('osml_extensions.detectors.async_sm_detector.S3Manager') as mock_s3_manager_class:
            with patch('osml_extensions.detectors.async_sm_detector.AsyncInferencePoller') as mock_poller_class:
                ...
                mock_sm_client.invoke_endpoint_async.return_value = {
...
class TestAsyncSMDetectorResourceManagement(unittest.TestCase):
    ...
    def test_context_manager_functionality(self, mock_boto3):
        ...
        with patch.object(detector, 'cleanup_resources') as mock_cleanup, \
             patch.object(detector.resource_manager, 'stop_cleanup_worker') as mock_stop:
    ...
    def test_destructor_cleanup(self, mock_boto3):
        ...
        with patch.object(detector.resource_manager, 'cleanup_all_resources') as mock_cleanup, \
             patch.object(detector.resource_manager, 'stop_cleanup_worker') as mock_stop:
    ...
    def test_resource_registration_in_find_features(self, mock_cleanup_policy_class, mock_boto3):
        ...
        with patch.object(detector, '_upload_to_s3', return_value="s3://input-bucket/input-key") as mock_upload, \
             patch.object(detector, '_invoke_async_endpoint', return_value="inference-123") as mock_invoke, \
             patch.object(detector, '_poll_for_completion', return_value="s3://output-bucket/output-key") as mock_poll, \
             patch.object(detector, '_download_from_s3', return_value=self.test_feature_collection) as mock_download, \
             patch.object(detector.s3_manager, 'generate_unique_key', side_effect=["input-key", "output-key"]) as mock_key_gen, \
             patch.object(detector.resource_manager, 'register_s3_object') as mock_register_s3, \
             patch.object(detector.resource_manager, 'register_inference_job') as mock_register_job:
    ...
    def test_failed_job_cleanup(self, mock_boto3):
        ...
        with patch.object(detector, '_upload_to_s3', return_value="s3://input-bucket/input-key"), \
             patch.object(detector, '_invoke_async_endpoint', return_value="inference-123"), \
             patch.object(detector, '_poll_for_completion', side_effect=AsyncInferenceTimeoutError("Timeout")), \
             patch.object(detector.s3_manager, 'generate_unique_key', side_effect=["input-key", "output-key"]), \
             patch.object(detector.resource_manager, 'register_s3_object'), \
             patch.object(detector.resource_manager, 'register_inference_job'), \
             patch.object(detector.resource_manager, 'cleanup_failed_job_resources') as mock_cleanup_failed:
