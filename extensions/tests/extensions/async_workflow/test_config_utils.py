...
from ..src.osml_extensions.config import (
...
class TestConfigurationValidator(unittest.TestCase):
            ...
            with patch.object(sagemaker_client, 'describe_endpoint_config') as mock_describe_config:
                mock_describe_config.return_value = {
                    'AsyncInferenceConfig': {
                        'OutputConfig': {'S3OutputPath': 's3://test-bucket/output/'}
                    }
                }
    ...
    def test_validate_sagemaker_endpoint_not_in_service(self):
        ...
        sagemaker_client.describe_endpoint.return_value = {
...
class TestEnvironmentConfigLoader(unittest.TestCase):
    ...
    def setUp(self):
        ...
        test_vars = [
            'TEST_STRING', 'TEST_INT', 'TEST_FLOAT', 'TEST_BOOL', 'TEST_LIST'
        ]
...
class TestConfigurationManager(unittest.TestCase):
    ...
    def test_load_and_validate_config_success(self):
        ...
        self.mock_validator.validate_s3_bucket_access.assert_any_call(
            'test-input-bucket', ['read', 'write', 'delete']
        )
        self.mock_validator.validate_s3_bucket_access.assert_any_call(
            'test-output-bucket', ['read', 'write', 'delete']
        )
