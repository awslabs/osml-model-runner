...
from osml_extensions.registry import HandlerMetadata, HandlerType, get_registry, register_handler, reset_registry
...
class RegistryTestUtils:
    ...
    def create_mock_handler(
        ...
        **kwargs,
        ...
        # Create mock class
        mock_class = type(
            f"Mock{name.title().replace('_', '')}", (), {"__init__": lambda self, **deps: setattr(self, "_deps", deps)}
        )
    ...
    @staticmethod
    def create_test_metadata(name: str, handler_type: HandlerType, handler_class: Type = None, **kwargs) -> HandlerMetadata:
        """
    ...
    @staticmethod
    def assert_handler_registered(request_type: str, handler_type: HandlerType, expected_name: str = None) -> None:
        """
        ...
        if expected_name:
            ...
            assert metadata.name == expected_name, f"Expected handler name '{expected_name}', got '{metadata.name}'"
    ...
    def get_registry_summary() -> dict:
        ...
        summary = {"stats": stats, "request_types": registry.get_supported_request_types(), "handlers": {}}
        ...
        for request_type in summary["request_types"]:
            ...
            summary["handlers"][request_type] = {
                handler_type.value: metadata.name for handler_type, metadata in handlers.items()
            }
