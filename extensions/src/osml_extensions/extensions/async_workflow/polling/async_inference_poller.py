...
class AsyncInferencePoller:
    ...
    def poll_until_complete(self, inference_id: str, metrics: Optional[MetricsLogger] = None) -> str:
        ...
        if isinstance(metrics, MetricsLogger):
            metrics.put_dimensions({
                "Operation": "AsyncInferencePolling",
                "InferenceId": inference_id[:8]  # Use first 8 chars for privacy
            })
        ...
        with Timer(
            task_str=f"Async Inference Polling",
            ...
            while True:
                try:
                    ...
                    if elapsed_time > self.config.max_wait_time:
                        ...
                        error_msg = (f"Async inference polling timed out after {elapsed_time:.1f} seconds "
                    ...
                    if job_status == "Completed":
                        if output_location:
                            ...
                            logger.info(f"Async inference job {inference_id} completed successfully after {attempt} polling attempts")
    ...
    def get_job_status(self, inference_id: str) -> Tuple[str, Optional[str]]:
        ...
        try:
            ...
            response = self.sm_client.describe_inference_recommendations_job(
                JobName=inference_id
            )
