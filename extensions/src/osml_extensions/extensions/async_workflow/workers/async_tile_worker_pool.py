...
from osml_extensions.utils import ResourceManager, ResourceType
...
class AsyncInferenceJob:
    ...
    def __init__(self, inference_id: str, tile_info: Dict[str, Any], 
                 input_s3_uri: str, output_s3_uri: str, submitted_time: float):
        """
...
class AsyncSubmissionWorker(Thread):
    ...
    def __init__(self, worker_id: int, tile_queue: Queue, job_queue: Queue, 
                 feature_detector: AsyncSMDetector, config: AsyncEndpointConfig,
    ...
    def run(self) -> None:
        ...
        try:
            ...
            logger.info(f"AsyncSubmissionWorker-{self.worker_id} finished. "
...
class AsyncPollingWorker(TileWorker):
    ...
    def __init__(self, worker_id: int, feature_table:FeatureTable, geolocator: Optional[Geolocator], 
                 region_request_table: RegionRequestTable, in_queue: Queue, result_queue: Queue,
                 feature_detector: AsyncSMDetector, config: AsyncEndpointConfig,
                 metrics_tracker: Optional[AsyncMetricsTracker] = None):
        """
    ...
    def run(self) -> None:
        ...
        try:
            ...
            logger.info(f"AsyncPollingWorker-{self.worker_id} finished. "
...
class AsyncTileWorkerPool:
    ...
    def __init__(self, sensor_model, elevation_model):
        """
        ...
        # Worker queues
        self.job_queue = Queue(maxsize=config.max_concurrent_jobs) # tile_queue
        ...
        self.resource_manager = ResourceManager(config, feature_detector.s3_client if hasattr(feature_detector, 's3_client') else None)
        
        logger.debug(f"AsyncTileWorkerPool initialized with {config.submission_workers} submission workers "
                    f"and {config.polling_workers} polling workers")
    ...
    def _start_workers(self, tile_queue: Queue) -> None:
        ...
        for i in range(self.config.submission_workers):
            ...
            if self.sensor_model is not None:
                geolocator = Geolocator(ImagedFeaturePropertyAccessor(), self.sensor_model, elevation_model=self.elevation_model)
        ...
        logger.info(f"Started {len(self.submission_workers)} submission workers "
                   f"and {len(self.polling_workers)} polling workers")
    ...
    def _process_result(self, result: Dict[str, Any]) -> None:
        ...
        logger.debug(f"Processed tile {tile_info.get('region')} with {feature_count} features "
