...
class ManagedResource:
    ...
    def __init__(self, resource_id: str, resource_type: ResourceType, 
                 resource_data: Dict[str, Any], created_time: Optional[float] = None):
        """
...
class ResourceManager:
    ...
    def __init__(self, config: AsyncEndpointConfig, s3_client=None):
        """
    ...
    def register_s3_object(self, s3_uri: str, cleanup_policy: Optional[CleanupPolicy] = None) -> str:
        ...
        resource = ManagedResource(
            ...
            resource_type=ResourceType.S3_OBJECT,
    ...
    def register_inference_job(self, inference_id: str, job_data: Dict[str, Any], 
                             cleanup_policy: Optional[CleanupPolicy] = None) -> str:
        """
        ...
        resource = ManagedResource(
            resource_id=resource_id,
            resource_type=ResourceType.INFERENCE_JOB,
            resource_data=resource_data
        )
    ...
    def register_worker_thread(self, thread: Thread, cleanup_policy: Optional[CleanupPolicy] = None) -> str:
        ...
        resource = ManagedResource(
            resource_id=resource_id,
            resource_type=ResourceType.WORKER_THREAD,
            resource_data=resource_data
        )
    ...
    def cleanup_all_resources(self, resource_type: Optional[ResourceType] = None, force: bool = False) -> int:
        ...
        """
        logger.info(f"Cleaning up all resources" + (f" of type {resource_type.value}" if resource_type else ""))
    ...
    def cleanup_failed_job_resources(self, inference_id: str) -> bool:
        ...
        s3_uris = [
            job_resource.resource_data.get("input_s3_uri"),
            job_resource.resource_data.get("output_s3_uri")
        ]
    ...
    def _schedule_cleanup(self, resource: ManagedResource, policy: CleanupPolicy) -> None:
        ...
        if policy == CleanupPolicy.IMMEDIATE:
            ...
            delayed_resource = {
                "resource": resource,
                "cleanup_time": time.time() + self.cleanup_delay_seconds
            }
