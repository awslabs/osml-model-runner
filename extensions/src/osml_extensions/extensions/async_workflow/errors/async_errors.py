...
class AsyncInferenceTimeoutError(AsyncInferenceError):
    ...
    def __init__(self, message: str, inference_id: Optional[str] = None, 
                 elapsed_time: Optional[float] = None, max_wait_time: Optional[float] = None):
        """
...
class S3OperationError(AsyncInferenceError):
    ...
    def __init__(self, message: str, operation: Optional[str] = None, 
                 s3_uri: Optional[str] = None, retry_count: Optional[int] = None):
        """
...
class AsyncEndpointError(AsyncInferenceError):
    ...
    def __init__(self, message: str, endpoint_name: Optional[str] = None,
                 error_code: Optional[str] = None, http_status_code: Optional[int] = None):
        """
...
class AsyncErrorHandler:
    ...
    @staticmethod
    def handle_client_error(error: ClientError, operation: str, 
                          context: Optional[Dict[str, Any]] = None) -> AsyncInferenceError:
        """
    ...
    def log_error_with_context(error: Exception, operation: str, 
        ...
        if isinstance(error, AsyncInferenceTimeoutError):
            logger.error(f"Async inference timeout in {operation}{context_str}: "
                        f"elapsed={error.elapsed_time}s, max={error.max_wait_time}s, "
                        f"inference_id={error.inference_id}")
        
        elif isinstance(error, S3OperationError):
            logger.error(f"S3 operation error in {operation}{context_str}: "
                        f"operation={error.operation}, s3_uri={error.s3_uri}, "
                        f"retries={error.retry_count}, error={str(error)}")
        
        elif isinstance(error, AsyncEndpointError):
            logger.error(f"Async endpoint error in {operation}{context_str}: "
            ...
            logger.error(f"Worker pool error in {operation}{context_str}: "
