...
class ConfigurationValidator:
    ...
    def validate_s3_bucket_access(self, bucket_name: str, required_permissions: list = None) -> bool:
        """
        ...
        try:
            ...
            if 'write' in required_permissions:
                try:
                    s3_client.put_object(
                        ...
                        Key=test_key,
                        Body=b"test",
                        Metadata={'test': 'async-endpoint-validation'}
                    )
                    logger.debug(f"Write permission validated for bucket {bucket_name}")
                except ClientError as e:
                    error_code = e.response.get('Error', {}).get('Code')
                    raise ExtensionConfigurationError(
                        f"No write permission for S3 bucket {bucket_name}: {error_code}"
                    )
            ...
            if 'read' in required_permissions:
                try:
                    ...
                    if error_code != 'NoSuchKey':  # NoSuchKey is expected if we didn't write
                        raise ExtensionConfigurationError(
                            f"No read permission for S3 bucket {bucket_name}: {error_code}"
                        )
            ...
            if 'delete' in required_permissions:
                try:
                    ...
                    error_code = e.response.get('Error', {}).get('Code')
                    raise ExtensionConfigurationError(
                        f"No delete permission for S3 bucket {bucket_name}: {error_code}"
                    )
        ...
        try:
            ...
            try:
                ...
                if error_code == 'ValidationException':
                    ...
                    raise ExtensionConfigurationError(
                        f"Error accessing SageMaker endpoint {endpoint_name}: {error_code}"
                    )
            ...
            raise ExtensionConfigurationError(
                f"Unexpected error validating SageMaker endpoint {endpoint_name}: {str(e)}"
            )
...
class EnvironmentConfigLoader:
    ...
    @staticmethod
    def get_int(key: str, default: Optional[int] = None, required: bool = False,
    ...
    @staticmethod
    def get_float(key: str, default: Optional[float] = None, required: bool = False,
    ...
    @staticmethod
    def get_list(key: str, default: Optional[list] = None, required: bool = False,
