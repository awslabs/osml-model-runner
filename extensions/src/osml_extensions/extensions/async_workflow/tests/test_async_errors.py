...
class TestAsyncErrors(unittest.TestCase):
    ...
    def test_async_inference_timeout_error(self):
        ...
        error = AsyncInferenceTimeoutError(
            message="Inference timed out", inference_id="test-inference-123", elapsed_time=300.5, max_wait_time=300.0
        )
    ...
    def test_s3_operation_error(self):
        ...
        error = S3OperationError(
            message="S3 upload failed", operation="upload", s3_uri="s3://test-bucket/test-key", retry_count=3
        )
    ...
    def test_worker_pool_error(self):
        ...
        error = WorkerPoolError(message="Worker failed", worker_type="submission", worker_id=1)
...
class TestAsyncErrorHandler(unittest.TestCase):
    ...
    def test_handle_s3_client_error(self):
        ...
        context = {"s3_uri": "s3://test-bucket/test-key", "retry_count": 2}
    ...
    def test_handle_sagemaker_client_error(self):
        ...
        client_error = ClientError(
            error_response={
                "Error": {"Code": "ValidationException", "Message": "Invalid endpoint name"},
    ...
    def test_handle_generic_client_error(self):
        ...
        client_error = ClientError(
            error_response={"Error": {"Code": "InternalError", "Message": "Internal service error"}},
    ...
    def test_is_retryable_error_client_error(self):
        ...
        # Retryable error
        retryable_error = ClientError(error_response={"Error": {"Code": "ServiceUnavailable"}}, operation_name="Test")
        self.assertTrue(AsyncErrorHandler.is_retryable_error(retryable_error))

        # Non-retryable error
        non_retryable_error = ClientError(error_response={"Error": {"Code": "ValidationException"}}, operation_name="Test")
    ...
    def test_log_error_with_context_timeout_error(self, mock_logger):
        ...
        error = AsyncInferenceTimeoutError(
            "Inference timed out", inference_id="test-123", elapsed_time=300.5, max_wait_time=300.0
        )
    ...
    def test_log_error_with_context_s3_error(self, mock_logger):
        ...
        error = S3OperationError("Upload failed", operation="upload", s3_uri="s3://test-bucket/test-key", retry_count=3)
