...
from osml_extensions.utils import ResourceManager, CleanupPolicy
...
class AsyncSMDetector(SMDetector):
    ...
    def __init__(self, endpoint: str, assumed_credentials: Optional[Dict[str, str]] = None, 
                 async_config: Optional[AsyncEndpointConfig] = None) -> None:
        """
        ...
        self.resource_manager = ResourceManager(self.async_config, self.s3_client)
    ...
    def find_features(self, payload: BufferedReader, metrics: MetricsLogger = None) -> FeatureCollection:
        ...
        try:
            ...
            with Timer(
                ...
                job_resource_id = self.resource_manager.register_inference_job(
                    inference_id, job_data, cleanup_policy
                )
    ...
    def _upload_to_s3(self, payload: BufferedReader, key: str, metrics: Optional[MetricsLogger]) -> str:
        ...
        """
        logger.debug(f"Uploading payload to S3 for async inference")
        return self.s3_manager.upload_payload(payload, key, metrics)
    
    def _invoke_async_endpoint(self, input_s3_uri: str, output_s3_uri: str, 
                              metrics: Optional[MetricsLogger]) -> str:
        """
    ...
    def _download_from_s3(self, output_s3_uri: str, metrics: Optional[MetricsLogger]) -> FeatureCollection:
        ...
        try:
            ...
            logger.debug(f"Successfully parsed FeatureCollection with {len(feature_collection.get('features', []))} features")
            return feature_collection
...
class AsyncSMDetectorBuilder:
    ...
    @classmethod
    def from_environment(cls, endpoint: str, assumed_credentials: Optional[Dict[str, str]] = None) -> "AsyncSMDetectorBuilder":
        """
