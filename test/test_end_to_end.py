...
class TestModelRunnerEndToEnd(TestCase):
    ...
    def test_create_elevation_model(self) -> None:
        """
        Test that the ModelRunner correctly creates an elevation model based on the Generic DEM tile set.
        ...
        assert elevation_model.tile_set.format_string == "%oh%od/%lh%ld.dt2"
        assert elevation_model.tile_set.min_latitude_degrees == -90.0
        assert elevation_model.tile_set.max_latitude_degrees == 90.0
