...
class TestSecurityClassification(TestCase):
    def test_valid_classifications(self):
        from aws_oversightml_model_runner.common import Classification, ClassificationLevel

        c1 = Classification(level=ClassificationLevel.UNCLASSIFIED)
        assert c1.classification == "UNCLASSIFIED"

        c2 = Classification(
            level=ClassificationLevel.UNCLASSIFIED, releasability="For Official Use Only"
        )
