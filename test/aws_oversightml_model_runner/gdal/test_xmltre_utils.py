...
@pytest.fixture
def sample_metadata_ms_rpc00b():
    with open("test/data/sample-metadata-ms-rpc00b.xml", "rb") as xml_file:
        xml_tres = ElementTree.parse(xml_file)
        return xml_tres


def test_get_tre_field_value_nominal(sample_metadata_ms_rpc00b):
    piaimc_tre = sample_metadata_ms_rpc00b.find("./tre[@name='PIAIMC']")
...
def test_parse_rpc_coefficients(sample_metadata_ms_rpc00b):
    rpc_tre = sample_metadata_ms_rpc00b.find("./tre[@name='RPC00B']")
...
def test_get_tre_field_value_invalid(sample_metadata_ms_rpc00b):
    piaimc_tre = sample_metadata_ms_rpc00b.find("./tre[@name='PIAIMC']")
...
def test_parse_rpc_coefficents_invalid_matching(sample_metadata_ms_rpc00b):
    rpc_tre = sample_metadata_ms_rpc00b.find("./tre[@name='RPC00B']")
    with pytest.raises(ValueError) as value_error:
...
def test_parse_rpc_coefficents_invalid_numbers(sample_metadata_ms_rpc00b):
    rpc_tre = sample_metadata_ms_rpc00b.find("./tre[@name='RPC00B']")
...
def test_parse_rpc_coefficents_invalid_group(sample_metadata_ms_rpc00b):
    rpc_tre = sample_metadata_ms_rpc00b.find("./tre[@name='RPC00B']")
    ...
    with pytest.raises(ValueError) as value_error:
        parse_rpc_coefficients(rpc_tre, "LINE_NUM_COEFF")
    assert "Field in repeated group in XML TRE is missing required value attribute" in str(
        value_error.value
    )
