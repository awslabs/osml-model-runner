...
class TestDDBHelper(TestCase):
    def setUp(self):
        ...
        from aws_oversightml_model_runner.database import JobItem
        from aws_oversightml_model_runner.database.ddb_helper import DDBItem, DDBKey

        self.ddb = boto3.resource("dynamodb", config=BotoConfig.default)
        self.table_name = TEST_ENV_CONFIG["JOB_TABLE"]
        ...
        ddb_range_key = DDBKey(
            hash_key="image_id", hash_value="12345", range_key="other_field", range_value="range"
        )
    ...
    def test_ddb_helper_put_ddb_item(self):
        from aws_oversightml_model_runner.database.ddb_helper import DDBHelper
        ...
        with self.assertRaises(ClientError):
            helper.put_ddb_item(
                self.job_item, condition_expression="attribute_not_exists(image_id)"
            )
    ...
    def test_ddb_helper_update_ddb_item_invalid_params(self):
        from aws_oversightml_model_runner.database import DDBUpdateException
        from aws_oversightml_model_runner.database.ddb_helper import DDBHelper
    ...
    def test_ddb_helper_get_update_params(self):
        ...
        update_expression, update_attributes = helper.get_update_params(
            self.job_item.to_update(), self.job_item
        )
        ...
        range_update_expression, range_update_attributes = helper.get_update_params(
            update_dict, self.range_job_item
        )
