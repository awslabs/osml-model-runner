...
class TestCompositeSensorModel(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.composite_sensor_model = CompositeSensorModel(
            self.approximate_sensor_model, self.precision_sensor_model
        )
    ...
    def test_world_to_image(self):
        assert np.array_equal(
            self.composite_sensor_model.world_to_image(
                GeodeticWorldCoordinate([0.1, 0.2, 0.3])
            ).coordinate,
            np.array([10.1, 11.1]),
        )
