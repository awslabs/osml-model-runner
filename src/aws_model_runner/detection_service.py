...
class FeatureDetector:
    ...
    def find_features(self, payload, metrics) -> FeatureCollection:
        logging.info("Invoking Model: {}".format(self.model_name))
        ...
        try:
            ...
            ml_endpoint_end = now()
            metrics.put_metric(
                "EndpointLatency", (ml_endpoint_end - ml_endpoint_start), "Microseconds"
            )
            ...
            # metrics.put_metric("ResultDecodeLatency", (decode_end - decode_start),
            # "Microseconds")
            ...
            logging.error("Unable to get detections from model.")
            logging.exception(de)
        except JSONDecodeError as de:
            self.error_count += 1
            metrics.put_metric("ModelErrors", 1, "Count")
            logging.error("Unable to decode response from model.")
            logging.exception(de)
