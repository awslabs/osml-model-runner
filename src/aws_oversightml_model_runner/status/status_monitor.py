...
from aws_oversightml_model_runner.common import ImageRequestStatus
...
class StatusMonitor:
    ...
    def process_event(
        self, image_request_item: JobItem, status: ImageRequestStatus, message: str
    ) -> None:
        ...
        if (
            ...
            raise StatusMonitorException(
                "StatusMonitor failed: {} {}".format(status, image_request_item.job_id)
            )
    ...
    def get_image_request_status(image_request_item: JobItem) -> ImageRequestStatus:
        ...
        :param image_request_item:

        :return: ImageRequestStatus
        """
        # Check that the image request has valid properties
        if (
