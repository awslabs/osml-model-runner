...
class Timer(object):
    def __init__(
        self, task_str, metric_name, logger: logging.Logger, metrics_logger: MetricsLogger = None
    ):
        self.str = task_str
    ...
    def set_use_milliseconds(self, use_milliseconds=True):
        self.use_milliseconds = use_milliseconds

    def set_normalizing_factor(self, normalizing_factor: float, normalizing_unit: str):
        ...
        try:
            if self.metrics_logger:
                ...
                if self.normalizing_factor and self.normalizing_unit:
                    ...
                    self.metrics_logger.put_metric(
                        f"{self.metric_name}Per{self.normalizing_unit}", time_per_unit, time_unit
                    )
                    self.logger.info(
                        f"{self.str} took {str(time_per_unit)} {time_unit.lower()} "
                        f"per {self.normalizing_unit}."
                    )
