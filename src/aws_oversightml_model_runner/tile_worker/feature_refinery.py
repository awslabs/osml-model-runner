...
class LocationGridInterpolator:
    ...
    def __init__(
        ...
        :param sensor_model: the sensor model for the image
        :param elevation_model: an optional external elevation model
        :param grid_area_ulx: the x component of the upper left corner of the grid in pixel space
        :param grid_area_uly: the y component of the upper left corner of the grid in pixel space
        :param grid_area_width: the width of the grid in pixels
        :param grid_area_height: the height of the grid in pixels
        :param grid_resolution: the number of points to calculate across the grid. Total points will be resolution^^2
        """
        ...
        for y in ys:
            for x in xs:
                world_coordinate = sensor_model.image_to_world(
                    ImageCoordinate([x, y]), elevation_model=elevation_model
                )
    ...
    def __call__(self, *args, **kwargs):
        ...
        image_coord = args[0]
        world_coord = [
            self.longitude_interpolator(image_coord[0], image_coord[1]),
            self.latitude_interpolator(image_coord[0], image_coord[1]),
            0.0,
        ]
...
class FeatureRefinery:
    ...
    def refine_features_for_tile(self, features: List[geojson.Feature]) -> None:
        ...
        :param features: the input features to refine
        :return: nothing, the features are updated in place
        """
    ...
    def _geolocate_individual_features(self, features: List[geojson.Feature]) -> None:
        ...
        :return: nothing but the individual features have their geometry property updated
        """
        for feature in features:
            ...
            if not isinstance(self.sensor_model, CompositeSensorModel):
                ...
                delta_longitude = (
                    final_center_location.longitude - approximate_center_location.longitude
                )
                delta_latitude = (
            ...
            # visualization tools.
            polygon_coords = [
                FeatureRefinery.radians_coordinate_to_degrees(image_coord)
                for image_coord in polygon_image_coords
            ]
