...
from aws_oversightml_model_runner.app_config import BotoConfig, MetricLabels, ServiceConfig
from aws_oversightml_model_runner.worker.feature_utils import create_mock_feature_collection
...
class FeatureDetector:
    ...
    def find_features(self, payload: BufferedReader, metrics: MetricsLogger) -> FeatureCollection:
        ...
        try:
            ...
            with Timer(
                ...
                if self.model_name == ServiceConfig.noop_model_name:
                    ...
                    metrics.put_metric(
                        MetricLabels.ENDPOINT_RETRY_COUNT, retry_count, str(Unit.COUNT.value)
                    )
        ...
        # If there was an error with the boto calls
            ...
            metrics.put_metric(error_code, 1, str(Unit.COUNT.value))
