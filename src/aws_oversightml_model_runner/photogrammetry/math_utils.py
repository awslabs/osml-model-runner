...
def equilateral_triangle(centroid: List[float], size: float) -> List[List[float]]:
    """
    Utility function to create an eqilateral triangle with a given side length centered on a point. There
    are many solutions to that problem so this returns the solution where the top corner is directly above
    the center and the base is horizontal. (i.e. for a triangle made of points ABC A[0] == center[0] and
    B[1] == C[1])
