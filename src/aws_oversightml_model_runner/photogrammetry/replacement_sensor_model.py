...
class RSMGroundDomain:
    """
    The RSM ground domain is an approximation fo the ground area where the RSM representation is valid. It is a solid
    ...
    def __init__(
        ...
        :param ground_domain_vertices: collection of vertices defining the area the model is valid within
        :param rectangular_coordinate_origin: optional origin of the rectangular coordinate system
        :param rectangular_coordinate_unit_vectors: optional unit vectors defining the rectangular coordinate system
        """
        if len(ground_domain_vertices) != 8:
            raise ValueError(
                f"Incorrect number of vertices in RSMGroundDomain. "
                f"{len(ground_domain_vertices)} provided, 8 expected."
            )
        ...
        if ground_domain_form == RSMGroundDomainForm.RECTANGULAR and (
            ...
            raise ValueError(
                "Rectangular coordinate system for RSMGroundDomain has not been defined"
            )
        ...
        if rectangular_coordinate_unit_vectors is not None:
            self.rectangular_coordinate_unit_vectors = np.array(
                rectangular_coordinate_unit_vectors, dtype=np.float64
            )
            self.rectangular_coordinate_unit_vectors_inverse = np.linalg.inv(
    ...
    def geodetic_to_ground_domain_coordinate(
        self, geodetic_coordinate: GeodeticWorldCoordinate
    ) -> WorldCoordinate:
        """
        ...
        if self.ground_domain_form == RSMGroundDomainForm.RECTANGULAR:
            ...
            if (
                self.rectangular_coordinate_origin is None
                or self.rectangular_coordinate_unit_vectors is None
            ):
            ...
            coordinate_relative_to_origin = (
                ecef_world_coordinate.coordinate - self.rectangular_coordinate_origin.coordinate
            )
    ...
    def ground_domain_coordinate_to_geodetic(
        self, ground_domain_coordinate: WorldCoordinate
    ) -> GeodeticWorldCoordinate:
        """
        This function converts a x, y, z coordinate defined in the ground domain of this sensor model into a WGS-84
...
class RSMContext:
    ...
    def __init__(self, ground_domain: RSMGroundDomain, image_domain: RSMImageDomain):
        """
        Construct the overall RSM context from from parameters typically found in the RSMID TRE.
...
class RSMLowOrderPolynomial:
    ...
    def __init__(self, coefficients: List[float]):
        ...
        if len(coefficients) != 10:
            raise ValueError(
                "Incorrect number of coefficients when constructing RSMLowOderPolynomial"
            )
...
class RSMSensorModel(SensorModel, ABC):
    ...
    def __init__(self, context: RSMContext):
        """
...
class RSMPolynomialSensorModel(RSMSensorModel):
    ...
    def world_to_image(self, geodetic_coordinate: GeodeticWorldCoordinate) -> ImageCoordinate:
        ...
        world_coordinate = self.context.ground_domain.geodetic_to_ground_domain_coordinate(
            geodetic_coordinate
        )
    ...
    def ground_domain_to_image(self, domain_coordinate: WorldCoordinate) -> ImageCoordinate:
        ...
        norm_row = self.row_numerator_poly(norm_domain_coordinate) / self.row_denominator_poly(
            norm_domain_coordinate
        )
        norm_column = self.column_numerator_poly(
            norm_domain_coordinate
        ) / self.column_denominator_poly(norm_domain_coordinate)
    ...
    def normalize_world_coordinate(self, world_coordinate: WorldCoordinate) -> WorldCoordinate:
        ...
        """
        norm_x = RSMPolynomialSensorModel.normalize(
            world_coordinate.x, self.x_norm_offset, self.x_norm_scale
        )
        norm_y = RSMPolynomialSensorModel.normalize(
            world_coordinate.y, self.y_norm_offset, self.y_norm_scale
        )
        norm_z = RSMPolynomialSensorModel.normalize(
            world_coordinate.z, self.z_norm_offset, self.z_norm_scale
        )
    ...
    def denormalize_world_coordinate(self, world_coordinate: WorldCoordinate) -> WorldCoordinate:
        ...
        """
        denorm_x = RSMPolynomialSensorModel.denormalize(
            world_coordinate.x, self.x_norm_offset, self.x_norm_scale
        )
        denorm_y = RSMPolynomialSensorModel.denormalize(
            world_coordinate.y, self.y_norm_offset, self.y_norm_scale
        )
        denorm_z = RSMPolynomialSensorModel.denormalize(
            world_coordinate.z, self.z_norm_offset, self.z_norm_scale
        )
    ...
    def normalize_image_coordinate(self, image_coordinate: ImageCoordinate) -> ImageCoordinate:
        ...
        """
        norm_line = RSMPolynomialSensorModel.normalize(
            image_coordinate.r, self.row_norm_offset, self.row_norm_scale
        )
        norm_samp = RSMPolynomialSensorModel.normalize(
    ...
    def denormalize_image_coordinate(self, image_coordinate: ImageCoordinate) -> ImageCoordinate:
        ...
        :return: the image coordinate (x, y)
        """
        denorm_line = RSMPolynomialSensorModel.denormalize(
            image_coordinate.r, self.row_norm_offset, self.row_norm_scale
        )
...
class RSMSectionedPolynomialSensorModel(RSMSensorModel):
    ...
    def __init__(
        ...
        row_section_size: int,
        column_section_size: int,
        ...
        :param context:
        :param row_num_image_sections:
        :param column_num_image_sections:
        :param row_section_size:
        :param column_section_size:
        :param row_polynomial:
        :param column_polynomial:
        :param section_sensor_models:
        """
        super().__init__(context)
    ...
    def world_to_image(self, geodetic_coordinate: GeodeticWorldCoordinate) -> ImageCoordinate:
        ...
        # Use the low order polynomials to approximate a x,y location of this world coordinate in the image
        domain_coordinate = self.context.ground_domain.geodetic_to_ground_domain_coordinate(
            geodetic_coordinate
        )
        ...
        column_section_index, row_section_index = self.get_section_index(
            approximate_image_coordinate
        )
    ...
    def get_section_index(self, image_coordinate: ImageCoordinate) -> Tuple[int, int]:
        ...
        containing the rough x, y. Note that these equations are slightly different than the documentation since
        ...
        :param image_coordinate:
        :return:
        """
        row_section_index = floor(
            (image_coordinate.y - self.context.image_domain.min_row) / self.row_section_size
        )
        ...
        column_section_index = floor(
            (image_coordinate.x - self.context.image_domain.min_column) / self.column_section_size
        )
