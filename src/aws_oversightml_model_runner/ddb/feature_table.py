...
from aws_oversightml_model_runner.app_config import BotoConfig, MetricLabels
from aws_oversightml_model_runner.worker.feature_utils import feature_nms
from aws_oversightml_model_runner.worker.image_utils import ImageDimensions
...
class FeatureTable:
    ...
    @metric_scope
    def add_features(self, features: List[Feature], metrics: MetricsLogger):
            ...
            for key, value in self.group_features_by_key(features).items():
                try:
                    ...
                    if result["ResponseMetadata"]["HTTPStatusCode"] != 200:
                        metrics.put_metric(MetricLabels.FEATURE_UPDATE, 1, Unit.COUNT.value)
                        metrics.put_metric(MetricLabels.FEATURE_ERROR, 1, Unit.COUNT.value)
                        logger.error(
                    ...
                    metrics.put_metric(MetricLabels.FEATURE_UPDATE_EXCEPTION, 1, Unit.COUNT.value)
                    metrics.put_metric(MetricLabels.FEATURE_ERROR, 1, Unit.COUNT.value)
    ...
    def get_dynamodb_resource() -> boto3.resource:
        return boto3.resource("dynamodb", config=BotoConfig.default)
