...
class ImageRequest(object):
    ...
    def __init__(self, *initial_data: Dict[str, Any], **kwargs: Any):
        ...
        :param initial_data: dictionaries that contain attributes/values that map to this class's attributes
    ...
    def from_external_message(image_request: Dict[str, Any]):
        """
        This method is used to construct an ImageRequest given a dictionary reconstructed from the JSON representation
        of a request that appears on the Image Job Queue. The structure of that message is generally governed by AWS
        API best practices and may evolve over time as the public APIs for this service mature.
...
def shared_properties_are_valid(request: Union[ImageRequest, RegionRequest]) -> bool:
    """
    There are some attributes that are shared between ImageRequests and RegionRequests. This function exists to
    :param request:
    :return:
    """
    ...
    if not request.model_hosting_type or request.model_hosting_type not in VALID_MODEL_HOSTING_OPTIONS:
        return False
    ...
    if (request.tile_overlap[0] < 0 or request.tile_overlap[0] >= request.tile_size[0]
            or request.tile_overlap[1] < 0 or request.tile_overlap[1] >= request.tile_size[1]):
        return False
