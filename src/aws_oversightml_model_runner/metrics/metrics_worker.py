...
class MetricsWorker(Thread):
    ...
    def run(self) -> None:
        while True:
            try:
                # Blocked get with a timeout of one second to avoid unnecessary tight loop when no
                # metrics are there in the input queue.
    ...
    def store_metrics(self, context: MetricsContext) -> None:
        ...
        try:
            ...
            if self.metrics_config.output_type == "cw":
                self.cw_client.put_metric_data(MetricData=metric_data, Namespace=context.namespace)
