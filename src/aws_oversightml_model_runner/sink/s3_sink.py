...
from .exceptions import InvalidS3BucketException
    ...
    def validate_s3_bucket(self) -> bool:
        ...
        try:
            ...
            if err.response["Error"]["Code"] == "404":  # Does not exist
                ...
                logging.error(
                    "Do not have permission to read/write this S3 Bucket({})".format(self.bucket)
                )
            raise InvalidS3BucketException(
                "Cannot read/write S3 Bucket ({})".format(self.bucket)
            ) from err
