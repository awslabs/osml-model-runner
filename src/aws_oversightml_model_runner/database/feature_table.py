...
from aws_oversightml_model_runner.common import ImageDimensions, Timer
from aws_oversightml_model_runner.inference import feature_nms
...
# TODO: Update and clean this up and implement DB helper to match JobTable
#  https://sim.amazon.com/issues/AnIMaL-24564
@dataclass
...
class FeatureTable(DDBHelper):
    def __init__(
        self, table_name: str, tile_size: ImageDimensions, overlap: ImageDimensions
    ) -> None:
        ...
        with Timer(
            ...
            for key, grouped_features in self.group_features_by_key(features).items():
                try:
                    ...
                    logger.debug(
                        f"Starting Add Features to DDB: {len(grouped_features)} "
                        f"features for {tile_id} {image_id}"
                    )
                    ...
                    for feature in grouped_features:
                        ...
                        if total_encoded_length > int(
                            ServiceConfig.ddb_max_item_size
                        ) or feature_count >= len(grouped_features):
            ...
            for group in grouped_items:
                ...
                for item in grouped_items[group]:
                    if item.features:
                        ...
                        logger.warning(
                            f"Found FeatureTable item: {item.range_key} with no features!"
                        )

                # If we are deduping do that here per tile
                if dedupe:
                    features.extend(feature_nms(batch_features))
                else:
