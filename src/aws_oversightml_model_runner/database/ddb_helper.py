...
class DDBItem:
    ...
    def to_put(self) -> dict:
        return {
            k: v for k, v in asdict(self).items() if v is not None and k not in self.__get_fields()
        }
...
class DDBHelper:
    ...
    def get_ddb_item(self, ddb_item: DDBItem) -> dict:
        """
    ...
    def put_ddb_item(self, ddb_item: DDBItem, condition_expression: str = None) -> dict:
        ...
        :return: dict: item from the put_item response
        """
        if condition_expression:
            response = self.table.put_item(
                Item=ddb_item.to_put(), ConditionExpression=condition_expression
            )
        else:
    ...
    def delete_ddb_item(self, ddb_item: DDBItem) -> dict:
        """
