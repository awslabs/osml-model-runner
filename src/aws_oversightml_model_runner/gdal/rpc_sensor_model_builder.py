...
from aws_oversightml_model_runner.photogrammetry import RPCPolynomial, RPCSensorModel, SensorModel
...
class RPCSensorModelBuilder(SensorModelBuilder):
    ...
    def build(self) -> Optional[SensorModel]:
        ...
        try:
            ...
            if success != 1:
                logging.info(
                    "RPC00B TRE SUCCESS field was not '1'. Skipping RPC sensor model build."
                )
                return None
            ...
            logging.warning(
                "Unable to parse RPC00B TRE found in XML metadata. No SensorModel created."
            )
    ...
    def build_rpc_polynomial(rpc_tre: ElementTree.Element, polynomial_name: str) -> RPCPolynomial:
        ...
        repeating groups of these coefficiencs for the polynomials associated with line or sample numerators and
        denominators.
