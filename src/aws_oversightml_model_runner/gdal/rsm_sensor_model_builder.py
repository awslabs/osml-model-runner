...
class RSMSensorModelBuilder(SensorModelBuilder):
    ...
    def __init__(self, xml_tres: ElementTree.Element):
        """
    ...
    def build(self) -> Optional[SensorModel]:
        ...
        :return: a RSM SensorModel if one can be constructed, None otherwise
        """
        ...
        try:
            ...
            rsm_polynomial_sensor_models = [
                RSMSensorModelBuilder._build_rsm_polynomial_sensor_model(rsmpc_tre, rsm_context)
                for rsmpc_tre in rsmpc_tres
            ]
            ...
            if len(rsm_polynomial_sensor_models) > 0:
                if len(rsm_polynomial_sensor_models) == 1:
                    ...
                    # TODO: Parse RSMPI and construct a segmented polynomial sensor model
                    logging.warning(
                        "Image has multiple RSMPCA TREs but RSM Segmented Sensor Models not implemented so no sensor "
                        "model returned! "
                    )
                    return None
            ...
            logging.warning(
                "Unable to parse RSM TREs found in XML metadata. No SensorModel created."
            )
    ...
    def _build_rsm_ground_domain(rsmid_tre: ElementTree.Element) -> RSMGroundDomain:
        ...
        if ground_domain_form == RSMGroundDomainForm.RECTANGULAR:
            ...
            return RSMGroundDomain(ground_domain_form, ground_domain_vertices)
    ...
    @staticmethod
    def _build_rsm_polynomial(
        rsmpc_tre: ElementTree.Element, polynomial_prefix: str
    ) -> RSMPolynomial:
        """
