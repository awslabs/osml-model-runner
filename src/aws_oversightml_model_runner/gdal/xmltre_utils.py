...
def get_tre_field_value(
    tre: ElementTree.Element, field_name: str, type_conversion: Callable[[str], T]
) -> T:
    """
...
def parse_rpc_coefficients(tre: ElementTree.Element, repeated_name: str) -> List[float]:
    ...
    if num_coefficients_str is None:
        raise ValueError(
            "Invalid XML TRE: Repeated tag in XML TRE is missing required number attribute"
        )
    ...
    for group in repeated:
        ...
        if index_str is None:
            raise ValueError(
                "Invalid XML TRE: Repeated group in XML TRE is missing required index attribute"
            )
        index = int(index_str)

        if group[0] is None:
            raise ValueError(
                "Invalid XML TRE: Repeated group in XML TRE is missing required field child element"
            )

        value_str = group[0].get("value")
        if value_str is None:
            raise ValueError(
                "Invalid XML TRE: Field in repeated group in XML TRE is missing required value attribute"
            )
